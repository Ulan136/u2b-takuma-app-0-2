  ...
  return (
    <div style={{ maxWidth:480, margin:'0 auto', fontFamily:"'Segoe UI',Arial,sans-serif", background:'#f0f2f8', minHeight:'100vh', paddingBottom:80 }}>
      ...
      <div style={{ padding:14 }}>
        <div style={{ background:'#fff', borderRadius:12, padding:14, marginBottom:12, boxShadow:'0 1px 4px #0001' }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
            ...
            <span style={{ background:sc.bg, color:sc.color, borderRadius:8, padding:'4px 12px', fontSize:12, fontWeight:700 }}>
              {order.status}
            </span>
          </div>
        </div>
        ...
        <div style={{ display:'flex', gap:8, marginBottom:12 }}>
          {[['view','👁 Просмотр','#1a1a4e'],['edit','✏️ Изменить','#ff9800'],['delete','🗑️ Удалить','#e53935']].map(([m,l,c])=>(
            <button key={m} onClick={()=>setMode(m)}
              style={{ flex:1, padding:'10px', border:'none', borderRadius:10, cursor:'pointer', fontWeight:700, fontSize:12,
                background:mode===m?c:'#e8e8f0', color:mode===m?'#fff':'#666' }}>
              {l}
            </button>
          ))}
        </div>
        ...
        <div style={{ background:'#fff', borderRadius:12, overflow:'hidden', boxShadow:'0 1px 4px #0001', marginBottom:12 }}>
          ...
          {items.length === 0 && <div style={{ textAlign:'center', padding:30, color:'#aaa' }}>Все позиции удалены</div>}
          {items.map((item, idx) => (
              ...
              <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                ...
                {mode==='edit' && (
                  <div style={{ display:'flex', alignItems:'center', gap:4, flexShrink:0 }}>
                    <button onClick={()=>changeQty(idx,item.qty-1)} style={{ width:32,height:32,border:'none',borderRadius:8,background:'#f0f0f0',fontWeight:900,fontSize:18,cursor:'pointer' }}>−</button>
                    ...
                    <button onClick={()=>changeQty(idx,item.qty+1)} style={{ width:32,height:32,border:'none',borderRadius:8,background:'#1a1a4e',color:'#fff',fontWeight:900,fontSize:18,cursor:'pointer' }}>+</button>
                    <button onClick={()=>removeItem(idx)} style={{ width:32,height:32,border:'none',borderRadius:8,background:'#fce4ec',color:'#e53935',fontWeight:900,fontSize:16,cursor:'pointer' }}>✕</button>
                  </div>
                )}
                {mode==='delete' && <div style={{ fontSize:20 }}>⚠️</div>}
              </div>
            </div>
          ))}
        ...
        {mode==='delete' && (
          <div style={{ background:'#fce4ec', borderRadius:12, padding:16, textAlign:'center' }}>
            <div style={{ fontSize:16, fontWeight:700, color:'#c62828', marginBottom:8 }}>⚠️ Удалить заказ №{String(orderNum).padStart(4,'0')}?</div>
            <div style={{ fontSize:13, color:'#888', marginBottom:16 }}>Это действие нельзя отменить</div>
            <div style={{ display:'flex', gap:8 }}>
              <button onClick={()=>setMode('view')} style={{ flex:1,padding:12,background:'#f0f0f0',border:'none',borderRadius:10,fontWeight:700,cursor:'pointer' }}>Отмена</button>
              <button onClick={deleteOrder} disabled={deleting} style={{ flex:1,padding:12,background:'#e53935',color:'#fff',border:'none',borderRadius:10,fontWeight:800,cursor:'pointer' }}>
      ...
      <div style={{ position:'fixed',bottom:0,left:0,right:0,background:'#fff',borderTop:'1px solid #e0e0e0',padding:'8px 16px',display:'flex',justifyContent:'space-between',zIndex:100 }}>
        <button onClick={()=>router.push('/admin')} style={{ background:'#f0f0f0',border:'none',borderRadius:10,padding:'10px 20px',fontWeight:700,fontSize:14,cursor:'pointer' }}>← Назад</button>
        <button onClick={()=>window.scrollTo({top:0,behavior:'smooth'})} style={{ background:'#f0f0f0',border:'none',borderRadius:10,padding:'10px 20px',fontWeight:700,fontSize:14,cursor:'pointer' }}>↑ Вверх</button>
      </div>
    </div>
  );
}
...
export default function OrderEditPage() {
  return (
    <Suspense fallback={<div style={{ display:'flex',justifyContent:'center',alignItems:'center',minHeight:'100vh' }}><div style={{ fontSize:40 }}>⏳</div></div>}>
      <OrderEditContent />
    </Suspense>
  );
}
